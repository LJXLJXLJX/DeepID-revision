...
#载入一个人的所有图片的路径 以列表形式输出
def fetch_pics_for_one_user(people_path):
    people_imgs = []
    ...
    random.shuffle(people_imgs)  #将列表中所有元素随机排序
    return people_imgs
...
def build_dataset(src_folder):
    ...
    for people_folder in os.listdir(src_folder):
        people_imgs = fetch_pics_for_one_user(os.path.join(src_folder, people_folder))
        total_people += 1
        total_picture += len(people_imgs)
        if len(people_imgs) < 100:
            #注意是append test_people最后是一个二阶列表 每个子列表是同一个人的图片集
